...
/**
 * Classe du client TCP à utiliser avec JMeter pour tester Camix.
 * 
 * <p>
 * Peut être utilisé pour tester le serveur Camix Java implémenté avec
...
public class TCPClientCamixImpl implements TCPClient {
	 ...
	 *
	 * <p>
	 * Une chaîne vide ne sera pas écrite.
	 * </p>
	 * <p>
	 * La chaîne écrite s'arrête avant le premier retour à la ligne (<tt>\n</tt>),
	 * si un retour à la ligne est présent.
	 * </p>
	 * <p>
	 * Remarque : l'envoi d'un chaîne vide est ainsi possible en saisissant
	 * seulement un retour à la ligne.
	 * </p>
	 * 
	 * @param os
	 *            le flux d'écriture de la socket.
	 * @param s
	...
	public void write(OutputStream os, String s) {
		try {
			if (s.length() != 0) {
				final BufferedWriter buffer = new BufferedWriter(new OutputStreamWriter(os, CHARSET));

				// Le message s'arrête avant le premier retour à la ligne,
				// si un retour à la ligne est présent.
				int limite = 0;
				while ((limite < s.length()) && (s.charAt(limite) != (char) this.eolByte)) {
					limite++;
				}
	 ...
	 *
	 * <p>
	 * La lecture se fait jusqu'à la levée d'une interruption d'expiration (timeout,
	 * sortie attendue) ou que la lecture retourne <tt>null</tt> (coupure de
	 * connexion).
	 * </p>
	 * 
	 * @param is
	 *            le flux de lecture de la socket.
	 * @param sr
	 *            le sample result.
	 * @return la chaîne de caractères.
	 */
	@Override
	public String read(InputStream is, SampleResult sr) {
		...
		try {
			...
			// pour fixer le temps de la première réponse
			boolean first = true;
	 ...
	 * 
	 * @param os
	 *            le flux d'écriture d'une socket.
	 * @param is
	 *            le flux de lecture d'une socket.
	 */
	@Override
	...
	/**
	 * Donne le caractère de fin-de-ligne/fin-de-message
	...
	/**
	 * Fixe le caractère de fin-de-ligne/fin-de-message
	 * (end-of-line/end-of-message).
	 *
	 * @param eolByte
	 *            le caractère à fixer.
	 */
	@Override
