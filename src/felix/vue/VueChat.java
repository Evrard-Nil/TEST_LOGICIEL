 ...
 * 
 * Cette vue est une vue active : elle possède une méthode d'activation qui
 * lance un thread de réception des messages du chat.
 * 
 * @version 3.0
...
public class VueChat extends VueFelix implements ActionListener, Runnable {
	...
	private void construireControles(Integer largeur, Integer hauteur) {
		...
		this.texteMessage.setName(Felix.CONFIGURATION.getString("TEXT_MESSAGE_NAME"));
		this.panMessage.add(this.texteMessage);

		/* chat messages */
		this.texteChatMessages = new JTextPane();
		this.texteChatMessages.setEditable(false);
		this.texteChatMessages.setName(Felix.CONFIGURATION.getString("TEXT_CHAT_NAME"));
	...
	public void run() {
		try {
			while (true) {
				...
				this.documentChatMessages.insertString(this.documentChatMessages.getLength(),
						message + System.getProperty("line.separator"), this.documentChatMessages.getStyle(STYLENAME));
